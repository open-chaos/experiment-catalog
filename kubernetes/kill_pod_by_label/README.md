...
* Environment Variables
  ...
  *  `POD_LABEL` - Specifies the label used to identify the pod that you are going to kill
...
> ***NOTE:*** The APPLICATION_ENTRYPOINT_URL & POD_LABEL should be setup according to your Kubernetes run time environment. The docker command also uses the .chaostoolkit/settings.yaml from the users home directory.
...
$ docker run -it \
           ...
           -e POD_LABEL=app=starlette; \
           -v `pwd`:/tmp/result \
           -v ~/.chaostoolkit:/tmp/settings \
           chaostoolkit/chaostoolkit \
           --settings /tmp/settings/settings.yaml \
           run https://raw.githubusercontent.com/open-chaos/experiment-catalog/master/kubernetes/kill_pod_by_label/kill_pod_by_label_experiment.json
```
...
(chaostk) $ export APPLICATION_ENTRYPOINT_URL=http://192.168.99.100/myapp; \
          $ export POD_LABEL=myapp; \
                   chaos run kill_pod_by_label_experiment.json
```
...
$ docker run -it \
           ...
           chaostoolkit/chaostoolkit \
           --settings /tmp/settings/settings.yaml \
           run kill_pod_by_label_experiment.json
```
